...
## Fase 6 — Resto de hallazgos ya identificados (limpieza) — 2 de 5 cerrados
...
- [~] Rol de acceso, según decisión ⚠️4 — **resuelto parcialmente**, ver "Auditoría de código y fixes de seguridad" arriba: el gate a nivel de *campo* (costo/ganancia) y de *scope* (vendedor solo ve sus propias operaciones) ya están. Sigue abierto el gate a nivel de *ruta* — cualquier usuario autenticado puede entrar por URL directa a `/reportes/rastreo-operaciones`; el control de acceso hoy vive solo en ocultar la card en `Reportes/Index.tsx` para vendedor, no en la ruta misma.
