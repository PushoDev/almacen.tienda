...
### Alta prioridad
- [ ] **"Atendido por" / Turnos — PRIMERA PRIORIDAD confirmada por el cliente para hoy (2026-09-08)**: diseño 100% resuelto en conversación (rondas del 2026-09-05 y 2026-09-07 — la del 07 reemplaza partes de la del 05), nada implementado todavía (solo un stub de migración `create_turnos_vendedor_table`, generado y borrado el mismo 2026-09-05). Alcance confirmado: bloqueo **global por middleware** (solo `moderador`/`vendedor`, nunca `admin`) hasta capturar el nombre de quien está en turno — no solo bloquea vender, bloquea navegar a cualquier página del sistema. Picker "seleccionar del historial o crear nuevo" (no autocompletado simple del valor de ayer), con **cambio de persona a mitad de turno** soportado — implica una tabla append-only `turnos_vendedor(user_id, nombre_vendedor, iniciado_en)`, no una fila por día. Cada `venta` debe guardar `turno_vendedor_id` (el turno activo al momento de crearla) para que Rastreo de Operaciones pueda mostrar quién atendió realmente, no solo qué cuenta de usuario/POS se usó. Indicador persistente del turno activo en el header de cada página (mismo mecanismo de prop compartida Inertia que ya usa `TasasFlotante`). Estimado dado al cliente: 2.5-3.5h de trabajo enfocado — el mayor riesgo es el alcance del middleware global (toca todas las rutas de esos 2 roles). Detalle completo en memoria `project_atendido_por_idea`.
