...
- [ ] Frontend: agregar a `RastreoOperaciones.tsx` — input de fecha único (reemplaza Desde/Hasta), `<Select>` de Tipo de Operación, campo de texto Buscar, y mostrar `monto_destino` (formato "≈ $X moneda @ tasa", como en `Cierres/Show.tsx`) en el detalle de Transferencia cuando aplica.
