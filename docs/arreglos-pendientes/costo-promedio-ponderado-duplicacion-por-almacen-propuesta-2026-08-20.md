...
**Estado: Fase 1 (Compras) SUPERADA 2026-09-18 por una regla más estricta. Fases 2-5 sin empezar.** Reemplaza el enfoque de `costo-promedio-ponderado-por-almacen-propuesta-2026-08-15.md` (v1, ver nota de reemplazo al inicio de ese doc). Este doc es v3 de la misma conversación del 2026-08-20 — v2 proponía que el disparador de duplicación fuera el almacén; en la misma sesión, el cliente lo corrigió: **el disparador real es el precio**, no el almacén. Este doc reemplaza el razonamiento de v2 con el modelo correcto y agrega el plan de fases y las vistas nuevas.

> **Actualización 2026-09-18 — la regla de match-por-precio de la Fase 1 ya no aplica.** Un cliente real reportó exactamente el escenario que la Fase 3 (abajo) preveía sin resolver: comprar "el mismo" producto (mismo precio) reutilizaba la ficha existente, y prorratear la compra nueva mutaba el costo de esa ficha compartida — afectando también el stock viejo que nunca fue parte de esa compra ni de ese prorrateo. Decisión del cliente: **cada compra crea siempre una ficha nueva, sin buscar match por ningún campo, ni siquiera precio** — el motivo original ("el precio es parte de la identidad") seguía dejando compartir ficha entre compras distintas cuando coincidían exacto, que es justo el caso que rompía. Implementado en `CompraController::procesarLineasProducto()` + guard nuevo en `DistribucionCostosController` (`estado === 'aprobada'` obligatorio para prorratear). Ver la fila de `docs/ESTADO_DESARROLLO.md` del 2026-09-18 para el detalle completo. La sección "La regla de identidad, corregida" de abajo queda como registro histórico de cómo se llegó hasta acá — no describe el comportamiento actual.
