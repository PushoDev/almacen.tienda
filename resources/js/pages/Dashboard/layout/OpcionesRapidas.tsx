...
export default function OpcionesRapidas({ userRole }: { userRole: 'admin' | 'moderador' | 'vendedor' }) {
    return (
        <>
            ...
            {/* Opciones */}
            <div
                className={`animate__animated animate__flipInX grid auto-rows-min gap-4 ${userRole !== 'vendedor' ? 'md:grid-cols-4' : 'md:grid-cols-3'}`}
            >
                {/* Widget de Compra - Admin y Moderador (vendedor no tiene acceso a Compras) */}
                {userRole !== 'vendedor' && (
                    <div>
