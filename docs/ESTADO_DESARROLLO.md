...
- [ ] **Idea del cliente, sin especificar todavía: sacar el precio de costo de un producto a su propio modelo** — hoy `precio_compra_producto` es una columna plana en `Producto`. Pedido 2026-08-20 mientras se cerraba el módulo Distribución de Costos, explícitamente "para luego" — no arrancar sin que el cliente aterrice el diseño (¿modelo por producto con historial, o algo más cercano a lo que ya hace `costo_historials`/`CostDistributionItem`? ¿reemplaza la columna o convive con ella?). Ver `docs/arreglos-pendientes/distribucion-costos-modulo-2026-08-20.md` y memoria `project_costo_promedio_ponderado` para el contexto relacionado (identidad de producto por precio, Fases 2/4/5 pendientes del mismo hilo).
