...
- [ ] **Idea del cliente: `Cuentas/Index.tsx` pasa de tabla a grilla de "tarjetas de banco"** — pedido 2026-08-25, cliente quiere empezar lo antes posible. Cada cuenta se muestra como una card con el gradiente/logo del banco real (BANDEC/BPA/Metropolitano/Clásica/Tropical, imágenes ya importadas en `public/projects/card_cubans/` + `card_interacionales/` para redes Visa/Mastercard), mismo lenguaje visual que la idea ya parqueada de "Detalles de Pago" (ver fila de arriba). 3 decisiones de diseño ya confirmadas (número de tarjeta decorativo autogenerado, cuentas efectivo con card genérica en el mismo formato, badge de categoría reusa `tipo_titular`) — falta la más grande: qué banco tiene cada una de las 80 cuentas tipo tarjeta reales (hoy no hay ningún dato que lo indique, reverificado 2026-09-08). **Ya preparado en el backend:** columna `cuentas.tipo` redujo su enum a solo `tarjeta`/`efectivo` (se quitó `otro`, sin uso real) y se agregó `cuentas.imagen` (string nullable, reverificado 2026-09-08 que sigue sin ningún consumidor en frontend/backend). Detalle completo, incluida la aclaración de que ambos catálogos de imágenes siguen creciendo (no hardcodear la lista actual), en memoria `project_cuentas_rediseno_tarjetas_bancarias`.
