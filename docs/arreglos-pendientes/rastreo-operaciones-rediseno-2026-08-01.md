...
- [x] **Tabla reorganizada a formato "partida doble"** (qué salió / qué entró) en vez de una sola columna `Monto`. Orden final: `Referencia | Fecha/Hora | Cuenta Envía | Monto | Cuenta que Recibe | Monto | Tasa de la Operación | Detalles`. Todo se lee de datos que ya estaban en el payload de cada fila (`detalle_movimiento.origen/destino`, `detalle_venta.pagos`) — no fue necesario tocar `RastreoOperacionesController.php`.
  ...
  - **Cuenta que Recibe:** Gasto/Ingreso/Transferencia muestran el nombre de `detalle_movimiento.destino` (`—` en Gasto, que no tiene destino). **Venta es especial:** una venta puede pagarse con varias cuentas/clientes a la vez, así que en vez de un solo nombre se renderiza **un `Badge` por cada pago** de `detalle_venta.pagos`, formato `"{cuenta/cliente}: {monto} {moneda}"`, sin agrupar aunque dos pagos vayan a la misma cuenta (decisión explícita del cliente — fiel al detalle, sin recalcular sumas).
  - **Monto (recibe):** en Transferencia usa `monto_destino` cuando hubo conversión real (si no, el mismo monto de siempre); en Ingreso es el monto de la operación; `—` en Gasto y en Venta (ahí el monto ya va dentro de cada badge, mostrarlo de nuevo sería redundante).
  - **Tasa de la Operación:** solo Transferencia con conversión real de moneda. `—` en Gasto, Ingreso y **también en Venta** (decisión explícita del cliente — una venta puede tener varios pagos con tasas distintas cada uno, mostrar una sola tasa a nivel fila sería ambiguo; la tasa de cada pago individual ya está en el detalle expandido).
  - Helpers nuevos en el archivo: `cuentaEnvia()`, `montoEnvia()`, `cuentaRecibeMovimiento()`, `montoRecibeMovimiento()`, `tasaOperacion()`.
...
- Verificado: `npm run types` limpio en este archivo (los únicos errores de tsc son los preexistentes de `pages/categorias/` vs `Categorias/` etc., ninguno tocado en esta sesión).
- **No verificado todavía en navegador** — cambio hecho y type-checkeado, pero sin probar la UI real contra datos.
