        ...
        <motion.div className={cn('space-y-10', className)} {...props}>
            <LayoutGroup>
                <div>
                    <AnimatePresence>
                        {pinned.length > 0 && (
                            <motion.p
                                layout
                                key="pinned-label"
                                className={cn('mb-2 px-3 text-sm font-medium text-neutral-500 dark:text-neutral-300', labelClassName)}
                                {...labelMotionProps}
                            >
                                {labels.pinned}
                            </motion.p>
                    ...
                    {pinned.length > 0 && (
                        <div className={cn('relative space-y-3', togglingGroup === 'pinned' ? 'z-5' : 'z-10', pinnedSectionClassName)}>
                            {pinned.map((item) => (
                                <motion.div
                                    key={item.id}
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-xs font-medium text-neutral-500 dark:text-neutral-400">{item.info}</div>
                                        </div>
                                    </div>
                ...
                <div>
                    ...
                    {unpinned.length > 0 && (
                        <div className={cn('relative space-y-3', togglingGroup === 'unpinned' ? 'z-5' : 'z-10', unpinnedSectionClassName)}>
                            {unpinned.map((item) => (
                                <motion.div
                                    key={item.id}
                                    layoutId={`item-${item.id}`}
