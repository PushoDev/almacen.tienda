...
> **Nota (actualizada 2026-09-24):** `EnsureUserIsAdmin` (alias `admin`) y `EnsureUserIsAdminOnly` (`admin.only`) **sí se aplican a rutas** (reportes, calendario, monedas, etc.). `EnsureUserIsModerator` y `EnsureUserIsVendor` **sí están registradas** como alias (`moderator`, `vendor`) en `bootstrap/app.php`, pero **no se aplican a ninguna ruta** (el cliente decidió dejarlas así). Los cuatro middlewares de rol, sin permiso, redirigen al `dashboard` con un aviso flash (sin sesión, al login); las peticiones JSON/Inertia reciben 403. Antes redirigían a `route('vendedor')`, que no existe (500). Solo `CheckAlmacenPermission` está realmente sin registrar (importada pero comentada en `bootstrap/app.php:31`). La verificación de roles se hace inline en los controladores mediante `$user->isAdmin()`, `$user->isModerador()`.
